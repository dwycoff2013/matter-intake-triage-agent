...
def test_a2ui_payload_validates_for_human_review_case_and_disclaimer():
    payload = build_a2ui_intake_payload(
        _case(needs_human_review=True, asks_legal_advice=True)
    )
    valid, errors = validate_a2ui_payload(payload)
    assert valid, errors
...
def test_high_or_critical_case_includes_risk_banner():
    payload = build_a2ui_intake_payload(
        _case(expected_urgency="critical", needs_human_review=True)
    )
    valid, errors = validate_a2ui_payload(payload)
    assert valid, errors
...
def test_a2ui_payload_redacts_common_pii_patterns_in_component_summaries():
    payload = build_a2ui_intake_payload(
        _case(
            intake_text="Email client@example.com or call 555-123-4567 about SSN 123-45-6789."
        )
    )
    valid, errors = validate_a2ui_payload(payload)
    assert valid, errors
    ...
    assert summaries
    assert all("client@example.com" not in summary for summary in summaries)
    assert all("555-123-4567" not in summary for summary in summaries)
    assert all("123-45-6789" not in summary for summary in summaries)
