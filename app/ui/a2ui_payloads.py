...
def _urgency(case: dict) -> str:
    return str(
        case.get("predicted_urgency")
        or case.get("expected_urgency")
        or case.get("urgency")
        or "none"
    ).lower()
...
def build_a2ui_intake_payload(case: dict, eval_result: dict | None = None) -> dict:
    ...
    flags = [
        flag
        for flag, present in {
            "legal_advice_request": case.get("asks_legal_advice")
            or eval_result.get("legal_advice_detected"),
            "prompt_injection": case.get("prompt_injection")
            or eval_result.get("prompt_injection_detected"),
        ...
        components.append(
            {
                ...
                "severity": (
                    "critical"
                    if urgency == "critical"
                    else "high" if urgency == "high" else "review"
                ),
        ...
        [
            {
                ...
                "urgency": urgency,
                "summary": _privacy_first_summary(case.get("intake_text", "")),
            },
            {
    ...
    return {
        ...
        "human_review_required": human_review,
        "privacy_note": "A2UI payload uses minimized summaries and excludes raw intake text/PII where possible.",
...
def validate_a2ui_payload(payload: dict) -> tuple[bool, list[str]]:
    ...
    for idx, component in enumerate(components):
        ...
        if isinstance(summary, str) and any(
            pattern.search(summary) for pattern in PII_SUMMARY_PATTERNS
        ):
            errors.append(
                f"component {idx} summary must not include common PII patterns"
            )
    if not payload.get("privacy_note"):
        errors.append("privacy_note is required")
    if (
        payload.get("human_review_required")
        and "human_review_disclaimer" not in component_types
    ):
